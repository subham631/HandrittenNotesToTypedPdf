...
const FileUploader = ({ setDownloadUrl, isProcessing, setIsProcessing }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Step 2: Generate PDF with extracted text
      const pdfBlob = await generatePDF(extractedText);

      // Step 3: Create a download URL
  ...
  async function generatePDF(text) {
    ...
    for (let word of words) {
      if (customFont.widthOfTextAtSize(line + " " + word, fontSize) > width - 2 * margin) {
